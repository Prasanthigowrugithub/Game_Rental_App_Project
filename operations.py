...
def Add_item_to_cart(Username,product_ID,Quantity,gamers_json_file,booking_start_date,booking_end_date,products_json_file):
    ...
    try:
        with open(gamers_json_file, 'r+') as file:
            gamer_data = json.load(file)
        with open(products_json_file, 'r+') as file_2:
            product_data = json.load(file_2)
            
            for gamer in gamer_data:
                if gamer['Username'] == Username:
                    for product in product_data:
                        if product['Product ID'] == product_ID:
                            if product['Total Stock Available'] >= Quantity:
                                cart = {
                                ...
                                gamer['Cart'].append(cart)
                                with open(gamers_json_file, 'w') as f:
                                    json.dump(gamer_data, f)
                                return True
            return False
